...
figure(Ch+1), clf
PowerdB=[0 -8 -17 -21 -25]; % Channel tap power profile 'dB'|�ŵ���ͷ�������� 'dB'
Delay=[0 3 5 6 8];          % Channel delay 'sample'|�ŵ�ʱ�ӣ������㣩
...
Nbps=4; M=2^Nbps;   % Modulation order=2/4/6 for QPSK/16QAM/64QAM|���ƽ���=2/4/6��QPSK/16QAM/64QAM
Nfft=64;            % FFT size|FFT��С
Ng=Nfft/4;             % Ng=0: Guard interval length|GI�ĳ��ȣ�û�б������ʱ��Ng=0
Nsym=Nfft+Ng;      % Symbol duration|��������
Nvc=Nfft/4;        % Nvc=0: no virtual carrier|Nvc=0��û��VC
Nused=Nfft-Nvc;
 
EbN0=[0:5:30];    % EbN0
N_iter=1e5;       % Number of iterations for each EbN0|����ÿһ��EbN0�ĵ�������
Nframe=3;         % Number of symbols per frame|ÿһ֡�ķ�����
...
for i=0:length(EbN0)
   ...
   Ber=0;%ԭʼ����ΪBer2=ber();�޷����� % BER initialization |��ʼ��BER
   ...
   for m=1:N_iter
      ...
      if NgType~=2, zx_GI=zeros(1,Nframe*Nsym);
       elseif NgType==2, x_GI= zeros(1,Nframe*Nsym+Ng);
        % Extend an OFDM symbol by Ng zeros |��Ng������չOFDM����
      end
      ...
      if Ch==0, y= x_GI;  % No channel|û���ŵ�
       else  % Multipath fading channel|�ྶ˥���ŵ�
        channel=(randn(1,Ntap)+j*randn(1,Ntap)).*sqrt(Power/2);
        h=zeros(1,Lch); h(Delay+1)=channel; % cir: channel impulse response|�ŵ�������Ӧ
        y = conv(x_GI,h); 
      end
      if i==0 % Only to measure the signal power for adding AWGN noise|ֻ�����źŹ���
        y1=y(1:Nframe*Nsym); sigPow = sigPow + y1*y1';
        continue;
      end
      % Add AWGN noise________________________________________________|��AWGN����
      snr = EbN0(i)+10*log10(Nbps*(Nused/Nfft)); % SNR vs. Eb/N0|ʽ4.28����Ƶ��SNR��ʱ��SNR
      ...
      if Ch==1
         H= fft([h zeros(1,Nfft-Lch)]); % Channel frequency response|�ŵ�Ƶ����Ӧ
